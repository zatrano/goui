...
- `PendingMounts` park/adopt (no double `Mount` on SSR → WS)
- `BaseComponent.Refresh` / `Session.Refresh` for async fill after Mount
- `PendingMounts.Stats()` (Parked / Adopted / Expired / Rejected)
